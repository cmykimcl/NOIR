#!/usr/bin/env python
"""
Train all Darcy models with multiple seeds.
...
def train_model(model, train_loader, test_loader, config, model_type, seed):
    ...
    # Different weight decay for FNO vs DeepONet
    if model_type in ['fourier', 'fourier_layer_ortho', 'wavelet']:
        weight_decay = config['training']['weight_decay_fno']
    else:
    ...
    train_data_res = config['data']['resolution'] ** 2
